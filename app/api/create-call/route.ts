...
function normalizePhoneNumber(phone: string): string {
  // Check if the original input had a plus sign (indicating international format)
  const hadPlusSign = phone.includes("+")
  
  // Remove all non-digit characters (including all plus signs to avoid duplicates)
  let cleaned = phone.replace(/[^\d]/g, "")

  // If the original had a plus sign, it was likely an international number
  // Return it with a single plus sign
  if (hadPlusSign) {
    return "+" + cleaned
  }
